...
# `CANCEL_BUTTON` берется из config.py
def cansel(message):
...
def start_command(message):
    bot.send_message(message.chat.id, f"""{EMOJI_START} Привет! Я бот-менеджер проектов
...
f"""
{EMOJI_INFO} Вот команды которые могут тебе помочь:

{EMOJI_PROJECT} /new_project - используй для добавления нового проекта
{EMOJI_SKILL} /skills - добавление навыков к проекту
{EMOJI_PROJECT} /projects - просмотр всех твоих проектов
{EMOJI_TRASH} /delete - удаление проекта
{EMOJI_EDIT} /update_projects - обновление информации о проекте
...
def name_project(message):
    ...
    data = [user_id, name]
    bot.send_message(message.chat.id, f"Введите ссылку на проект {EMOJI_LINK}")
    bot.register_next_step_handler(message, link_project, data=data)

def link_project(message, data):
    data.append(message.text)
    statuses = [x[0] for x in manager.get_statuses()] 
    bot.send_message(message.chat.id, f"Введите текущий статус проекта {EMOJI_EDIT}", reply_markup=gen_markup(statuses))
...
def callback_project(message, data, statuses):
    ...
    bot.send_message(message.chat.id, f"{EMOJI_CHECK} Проект сохранен")
...
def skill_handler(message):
    ...
    if projects:
        ...
        bot.send_message(message.chat.id, f'Выбери проект для которого нужно выбрать навык {EMOJI_PROJECT}', reply_markup=gen_markup(projects))
...
def set_skill(message, project_name, skills):
    ...
    bot.send_message(message.chat.id, f'{EMOJI_CHECK} Навык {skill} добавлен проекту {project_name}')
...
def delete_project(message, projects):
    ...
    bot.send_message(message.chat.id, f'{EMOJI_TRASH} Проект {project} удален!')
...
def update_project(message):
    ...
    if projects:
        projects = [x[2] for x in projects]
        bot.send_message(message.chat.id, f"{EMOJI_EDIT} Выбери проект, который хочешь изменить", reply_markup=gen_markup(projects))
...
def update_project_step_4(message, project_name, attribute): 
    ...
    bot.send_message(message.chat.id, f"{EMOJI_CHECK} Готово! Обновления внесены!")
...
def text_handler(message):
    ...
    if project in projects:
        ...
        return
    bot.reply_to(message, f"Тебе нужна помощь? {EMOJI_INFO}")
    info(message)
